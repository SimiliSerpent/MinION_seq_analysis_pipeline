...
def filter_GA(input, output, threshold=10, longest=False, verbosity=0):
    ...
    GA_filtering_dict         (dict) - dictionnary containing the number of
                                       filtered subsequences and bases
    """
    ...
    with open(input, 'r') as reads_in:
        ...
        with open(output, 'w') as reads_out:
            ...
            if not line:
                ...
                return(GA_repeats_lengths, GA_filtering_dict)
    ...
    GA_filtering_dict = {
        sample_name: {
            'GA_repeats': {
                'input_seq': nb_reads,
                'sequences': nb_trimmed_repeats,
                'bases': nb_trimmed_bases
            }
        }
    }
...
def main():
    ...
    GA_repeats_lengths, GA_filtering_dict = filter_GA(
    ...
    if args.stats_out is not None:
        utils.send_text(
            'Saving GA filtering stats to ' + args.stats_out, v, 1, 0
        )
        utils.save_json(args.stats_out, GA_filtering_dict)
