...
def get_samples_composition(samstats_summary_dir):
    ...
    # Loop through files and parse the names to find cross-species alignment stats
...
def plot_composition(samples, species, composition, bar_width, bar_colors,
    ...
    snr=False, y_label='Read count', verbosity=0):
    """Returns a dict containing the number of reads per sample for each target.
    ...
    snr              (bool) - use True to display snr on top of bars
    ...
    dummy_var = fig.set_size_inches(1+len(samples), 5)
    ...
    # initialize the bars max height to plot snr box if requested
    if snr:
        snrs_y = [0 for sample in samples]
    ...
    for specie in species:
        ...
        # update max bar height to plot snr box if requested (and if total nb
        # of reads is not displayed, otherwise the box is plot over that bar)
        if snr and not total:
            for sample_ind, sample in enumerate(samples):
                if dummy_var[sample_ind].get_height() > snrs_y[sample_ind]:
                    snrs_y[sample_ind] = dummy_var[sample_ind].get_height()
        multiplier += 1
    ...
    # Set x tick labels out of previous function call to allow multi-line
    ax.set_xticklabels(
        [s.replace('\\n', '\n') for s in samples],
        rotation=60,
        ha='right',
    ...
    if snr:
        if total:
            ...
            snrs_y = [bar.get_height() for bar in total_bars]
        # ...then add a lil something to put the box slightly over the bar
        ymin, ymax = ax.get_ylim()
        factor = (ymax / ymin) ** 0.05
        snrs_y = [y * factor for y in snrs_y]
        # add some boxes with the snr above the bars
        for s_i, sample in enumerate(samples):
            # compute snr
            snr_val = sum([composition[specie][s_i] for specie in species]) \
                      / composition['all'][s_i]
            # compute the x & y position of the boxes
            snr_x = s_i * ((len(species) + 2.5) * bar_width) + (len(species[:-1]) * bar_width / 2)
            snr_y = snrs_y[s_i]
            # add the box
            # (plot snr value with 4 significant digits)
            # f'SNR: {snr_val:.2f}',
            # (plot snr value - scientific writing)
            # f'SNR: {snr_val:.1E}',
            snr_box = ax.text(
                snr_x,
                snr_y,
...
def main():
    ...
    parser.add_argument('-r', '--snr', action='store_true',
                        help='Display snr on top of bars')
