...
def trim_tails(input, output, verbosity=0):
    ...
    tail_trimming_dict          (dict) - dictionnary containing the number of
        5' and 3' trimmed tails and bases
    """
    ...
    with open(input, 'r') as fastq_in:
        ...
        with open(output, 'w') as fastq_out:
            ...
            nb_trimmed_polyA = 0
            nb_trimmed_polyT = 0
            nb_trimmed_bases_end = 0
            nb_trimmed_bases_start = 0
            ...
            while line:
                ...
                # Trimm and detect coming sequencing score
                    ...
                    nb_trimmed_bases_end += right_trimmed
                    nb_trimmed_bases_start += left_trimmed
        ...
        if verbosity > 0:
            ...
            print(
                ...
                nb_trimmed_polyT,
                'polyT ending tail ->',
                nb_trimmed_bases_start,
                'bases.'
            )
    
    trimmed_tail_lengths.sort()

    sample_name = input.split('/')[-1].split('_porechopped_')[0].split('.')[0]
    tail_trimming_dict = {
        sample_name: {
            'polyA/T_tails': {
                'input_seq': nb_reads,
                'start_trim_seq': nb_trimmed_polyT,
                'start_trim_bases': nb_trimmed_bases_start,
                'end_trim_seq': nb_trimmed_polyA,
                'end_trim_bases': nb_trimmed_bases_end,
            }
        }
    }
    
    return(trimmed_tail_lengths, tail_trimming_dict)
...
def main():
    ...
    parser.add_argument('-o', '--out_fastq', type=str,
                        ...
                        + '>_no_tail.fastq)')
    parser.add_argument('-s', '--stats_out', type=str,
                        help='Path to the output json file with the statistics'
                        + ' of the trimmed tails')
    ...
    trimmed_tail_lengths, tail_trimming_dict = trim_tails(
        args.in_fastq, out_path, v
    )

    # Save statistics of tail trimming
    if args.stats_out is not None:
        utils.send_text(
            'Saving tail trimming stats to ' + args.stats_out, v, 1, 0
        )
        utils.save_json(args.stats_out, tail_trimming_dict)
