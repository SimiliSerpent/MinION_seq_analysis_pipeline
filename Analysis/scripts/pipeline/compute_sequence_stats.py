...
def main():
    ...
    parser.add_argument('-e', '--exp_dir', type=str, required=True,
                        ...
                        + "sam files; a 'demultiplexed' directory containing "
                        + 'the demultiplexed yet untrimmed fastq files; and a '
                        + "'trimmed_fastq' directory containing the "
                        + 'porechopped fastq files.')
    ...
    parser.add_argument('-o', '--output', type=str,
                        ...
                        + 'files with the pandas data frames for all barcodes '
    ...
    # Retrieve the barcodes names (cutting the size of '.fastq' = 6 letters
    # from the end of the path)
    utils.send_text(
        'compute_sequence_stats.py: Getting barcodes names', v, 1, 1
    )
    barcodes_paths = utils.find_path(
        args.exp_dir.rstrip('/') + '/demultiplexed/',
        '.fastq'
    )
    barcode_names = [r_path.split('/')[-1][:-6] for r_path in barcodes_paths]
    barcode_names.sort()
    utils.send_text('compute_sequence_stats.py: Found barcodes are: ' \
                    +  ', '.join(barcode_names), v, 2, 2)
    ...
    all_trim_states = utils.find_file(
        ...
        barcode_names[0] + '_porechopped_'
    )
    # Remove the name of the barcode from the beginning of the file name + the
    ...
    all_trim_states = ['untrimmed'] + [
        name[len(barcode_names[0])+13 : -6] for name in all_trim_states
    ...
    # Loop through barcodes and trimming states and build data frames
    for barcode in barcode_names:
        utils.send_text(
            'compute_sequence_stats.py: Analyzing ' + barcode, v, 1, 1
        )
        ...
        for trim_state in all_trim_states:
            ...
            # Eventhough this is infuriating, get rid of categorical in the
            # dataframe because pandas is currently bugged and cannot load
            # pickles with categorical, even those it has just created
            # .astype({
            #     'species': species_cat,
            #     'size': 'int64',
            #     'qual': 'float64'
            ...
            #     if pd.api.types.is_extension_array_dtype(qual_df[col].dtype):
            #         qual_df[col] = qual_df[col].astype(object)
            
            # Save data frame
            # stored_df_path = args.output.rstrip('/') + '/' + barcode + '_' \
            #                  + trim_state + '.pkl'
            # qual_df.to_pickle(stored_df_path)

            # Pandas is bugged and impossible to retrieve the data it saved,
            # so we use another library
            # (if pandas is patched, this can be removed along with the lib)
            with open(
                args.output.rstrip('/') + '/' + barcode + '_' \
                + trim_state + '.pkl',
                "wb"
            ) as output_path:
                pickle.dump(qual_df, output_path)
