...
Takes as input a '.pkl' file with all the information in the form of a pandas
data frame, i.e. reads lengths, the reads nucleotides frequencies, the reads
species it was assigned to, and the reads bases quality (Phred score), and
as second input the location of the pod5 read files, and outputs a text file
for each species witht the.
"""
...
def main():
    ...
    parser.add_argument('-p', '--pod5dir', type=str, required=True,
                        help='Path to the directory containing the pod5 input '
                        + 'files (can be in subdirectories, search for pod5 '
                        + 'is recursive).')
    parser.add_argument('-r', '--read_pickles', type=str, required=True,
                        help="Path to the directory with the '.pkl' "
                        + 'files with the pandas data frames for all samples '
                        + 'and trimming state inside (default: same directory '
                        + 'as experiment).')
    parser.add_argument('-o', '--output', type=str,
                        help='Path to the output directory containing the '
                        + "'.npy' (np array) files and the distributions for "
                        + 'Icarust.')
    ...
    command = 'get_reads_by_species.py' \
              + ' -p ' + args.pod5dir \
              + ' -r ' + args.read_pickles \
              + ' -o ' + args.output \
              + ' -v ' + str(v)
    ...
    # Get the IDs of the reads mapping to each species
    ids_df = {}

    # Get sample names
    samples_paths = utils.find_path(args.read_pickles, '_ONT_TSO_tail_GA.pkl')
    samples_names = [r_path.split('/')[-1][:-20] for r_path in samples_paths]

    # Loop through all samples
    for sample in samples_names:

        # Retrieve the pickle file with the trimmed reads infos
        reads_df = pd.read_pickle(
            args.read_pickles.rstrip('/') \
            + '/' \
            + sample \
            + '_ONT_TSO_tail_GA.pkl'
        )

        # Store the ids in the list of data frame
        ids_df[sample] = reads_df[reads_df['species'] != 'Unmapped']['species']
    
    # Loop through all pod5 files (sample by sample) and retrieve squiggle
    # (this comes from the code of Icarust issue #28)
    distributions = collections.defaultdict(list)

    for pod5_file in pathlib.Path(args.pod5dir).glob('*.pod5'):

        with pod5DatasetReader(str(pod5_file)) as reader:
            ...
            for read in reader.reads():
                ...
                for sample in sample_names:
                    if read_id in ids_df[sample].index:
        ...
        for trim_state in all_trim_states:
            ...
            with open(fname, 'r') as fastq_file:
                ...
                while line:
                    ...
                    reads_stats[read_id] = [
                        alignment_dict.get(read_id, 'Split'),
                        len(score),
                        sum(phred)/len(phred)
                    ]
